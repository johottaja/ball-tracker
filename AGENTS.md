...
- **`game_tracker/`** — production dual-camera app for recording a full beer pong game. It tracks one active throw at a time from either main-camera image half, triangulates 3D trajectories from calibrated camera geometry, and saves side-tagged throws to `game_tracker/games/*.json` for `throw_visualizer`. Records native `left.mp4` / `right.mp4` under `game_tracker/recordings/` plus `stereo_timeline.json` for time-domain stereo alignment (same module as `stereo_viewer`).
- **`pose_detection/`** — reusable YOLO pose pipeline: per-frame dominant-hand selection, fixed left/right player-slot selection, and batch extraction of arm keypoints from frame sequences.
...
│   ├── processor.py          # GameTrackingProcessor — two player slots, stereo tracking + JSON export
...
└── video_viewer/             # Viewer and CV debugging app
    ...
    ├── yolo_batch.py         # Batched YOLO pose inference + player-slot npz cache
    ├── gru_batch.py          # Batched GRU throw inference per player slot + npz cache
...
| `stereo_timeline.py` | `StereoTimeline` — validated bidirectional master slots from per-camera `time.monotonic()` capture timestamps; pairs nearby captures and holds either camera as needed; save/load `stereo_timeline.json` |
...
**Recording:** Raw frames to `game_tracker/recordings/left.mp4` and `right.mp4` at native frame counts. On stop, per-camera capture timestamps are saved to `stereo_timeline.json`. `StereoTimeline` forms timestamp-paired master slots only over the common capture interval: nearby captures share a slot; either side holds its latest native frame when the other advances. Playback and batch processing read those maps through `StereoFrameReader` without re-encoding. Playback can use a marked synthetic fallback if metadata is missing; 3D game processing refuses that fallback because it requires capture timestamps.

**Offline processing (`batch_process.process_game_recording`):** three phases — batched YOLO pose (`yolo_inferences.npz`) for fixed left/right player slots, independent batched GRU labels for those main-camera slots (`gru_inferences.npz`, keyed to the active `.pt` model), then ball/trajectory tracking with both caches loaded into `PlaybackCache`. The left slot negates only torso-normalized elbow/wrist x before GRU inference so the existing right-side model behavior is preserved. Older single-player caches are stale and are regenerated.
